...
class Common extends Frontend
{
    ...
    public function updatePrint()
    {
        ...
        $date['order_status'] = 3;//体检状态更改为已出卡
    ...
    public function inspect($type = '', $orderId = '')
    {
        ...
        // $hello  = db("order_detail")->alias('od')
        //         ->join("inspect i", 'od.item=i.id and od.physical_result_ext=i.id')
        //         //->join('inspect i','od.physical_result_ext=i.id')
        //         ->field("i.id,name,item,physical_result,physical_result_ext")
        //         ->where($where)
        //         ->group("name")
        //         ->select();
   ...
   //更新体检结果
    ...
    public function checkOrderStatus($orderId){
        //查询order_detail表，14项数据
         //physical_result 如果都为2，未体检；如果不都为2，体检完；其他的体检中
        $where['order_serial_number'] = $orderId;
        $where['odbs_id'] = $this->busId;
        $res = db('order_detail')->where($where)->select();//现在是14条
        $counts = db('order_detail')->where($where)->count();//记录条数，不写死

        $i = 0;//计数器    
        foreach($res as $v){       

            if($v['physical_result'] != 2){
                $i++;
            }     
