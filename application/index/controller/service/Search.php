 ...
 * */
    public function info11(){
        ...
        //查询inspect信息
        $orderDetail = OrderDetail::getInspectDetail(0,$order_id);
        $extDetail = OrderDetail::getExtDetail(0,$order_id);
        //print_r($extDetail);
        $inspectDetail = array();
        $extsDetail = array();
        foreach($orderDetail as $v){

              $inspectDetail[] = $v->toArray();
        }
        foreach($extDetail as $v){

              $extsDetail[] = $v->toArray();
        }
        print_r($extsDetail);
        $arr = array();
        foreach($inspectDetail as $v){
              $v['item'] = $v['inspect_detail']['name'];
              $arr[] = $v;
        }
        // //视图查询
        // $inspectDetail = Db::view('order_detail')
        // ->view('fa_inspect','id,name,type','order_detail.item=inspect.id')
    ...
    public function info(){
        ...
        $count = db("order")->where($countWhere)->count();
        //取得体检状态
        $order_detail = db("order")->where('order_serial_number','=',$info['order_serial_number'])->find();      
